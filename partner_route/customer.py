...
class account_mc_customer(osv.osv):
    _name = 'partner.mc.customer'
    _auto = False

    def read_group(self, cr, uid, domain, fields, groupby, offset=0, limit=None, context=None, orderby=False):
        if 'max_days_overdue' in fields:
            fields.remove('max_days_overdue')
        return super(account_mc_customer, self).read_group(cr, uid, domain, fields, groupby, offset, limit=limit, context=context, orderby=orderby)
...
#rut.codigo_mun,rut.nombre_mun,rut.codigo_dep,rut.nombre_dep,
    ...
    def init(self, cr):
        """
        @author       
        @description  Update table on load with latest mc information
        @modified  
        """

        query = """ 
                Select rut.codigo_mun,rut.nombre_mun,rut.codigo_dep,rut.nombre_dep,
        sq2.id,sq2.partner_id,sq2.partner_name,
...
days_due AS avg_days_overdue, NULL as date, date as date_due,                
               ...
               --------++--------   
	            ...
	            LEFT JOIN (Select m.id mun_id,m.departamento_id,m.codigo codigo_mun,m.nombre nombre_mun, d.codigo codigo_dep,d.nombre nombre_dep
				From mc_municipios m
				JOIN  mc_departamentos d
				ON m.departamento_id = d.id) rut
			ON rut.mun_id = rpc.municipio_id
              """
